...
def set_text(text_frame, text_value, font_size=None, is_warning=False):
    ...
    if text_frame.paragraphs:
        ...
        # Nível de parágrafo
        if para.font.name:
            tmpl_font_name = para.font.name
        if para.font.size:
            tmpl_font_size = para.font.size
        if para.font.bold is not None:
            tmpl_bold = para.font.bold
        # Nível de run tem prioridade
        if para.runs:
            run = para.runs[0]
            if run.font.name:
    ...
    logging.info(f"  Template: fonte='{tmpl_font_name}', tamanho={tmpl_font_size}, negrito={tmpl_bold}")

    text_frame.clear()
    p = text_frame.add_paragraph()

    # Alinhamento do template
    ...
    # Fonte do template (não sobrescreve com Liberation Sans)
    if tmpl_font_name:
        p.font.name = tmpl_font_name

    # Tamanho: só sobrescreve se explicitamente passado
    ...
    logging.info(f"  Texto definido: '{text_value}' | fonte='{p.font.name}' | tamanho={p.font.size}")
    ...
    try:
        ...
        def find_shape(slide_index, shape_name_to_find):
            if slide_index < 0 or slide_index >= len(prs.slides):
                logging.warning(f"{log_prefix} Slide índice {slide_index} não existe.")
                return None
            slide = prs.slides[slide_index]
            for shape in slide.shapes:
                if shape.name.strip().lower() == shape_name_to_find.strip().lower():
                    if shape.has_text_frame:
                        logging.info(f"{log_prefix}   Shape '{shape.name}' encontrada no slide {slide_index+1}.")
        ...
        # Tamanho grande para mensalidades (só sobrescreve nas slides de preço)
        ...
        set_text(find_shape(4, "adesão"), adesao_valor_str, font_size=TAMANHO_FONTE_MENSALIDADE)
        set_text(find_shape(4, "ouro"), format_currency_value_only(precos.get('Plano Ouro')), font_size=TAMANHO_FONTE_MENSALIDADE)
        ...
        set_text(find_shape(5, "diamante"), format_currency_value_only(precos.get('Diamante')), font_size=TAMANHO_FONTE_MENSALIDADE)

        logging.info(f"{log_prefix} Preenchendo Slide 7 (Índice 6) - Platinum")
        set_text(find_shape(6, "adesão"), adesao_valor_str, font_size=TAMANHO_FONTE_MENSALIDADE)
        set_text(find_shape(6, "platinium"), format_currency_value_only(precos.get('Platinum')), font_size=TAMANHO_FONTE_MENSALIDADE)
